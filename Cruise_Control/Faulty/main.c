...
//  NO protection — zero RPM accepted silently
// ─────────────────────────────────────────────
void vEncoderTask(void *pvParameters) {
...
//  NO protection — runs blindly regardless
//  of RPM value or task stalls
// ─────────────────────────────────────────────
void vCruiseTask(void *pvParameters) {
    TickType_t xLastWakeTime = xTaskGetTickCount();
...
//  NO protection — if task stalls, buttons
//  and pot stop responding permanently
// ─────────────────────────────────────────────
void vManualTask(void *pvParameters) {
    TickType_t xLastWakeTime = xTaskGetTickCount();
...
// ─────────────────────────────────────────────
//  TASK 4: LCD Task (Priority 1)
//  Updates display every 200ms
//  Line 1: current speed
//  Line 2: target speed + state
// ─────────────────────────────────────────────
void vLCDTask(void *pvParameters) {
    TickType_t xLastWakeTime = xTaskGetTickCount();
    char num_buf[4];
